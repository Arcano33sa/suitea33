...
assert(index.includes('rel="manifest" href="manifest.webmanifest?v=4.20.95&r=3"'), 'Manifest no enlazado');
assert(index.includes('style.css?v=4.20.95&r=16'), 'Revisión CSS final incorrecta');
assert(index.includes('app.js?v=4.20.95&r=20'), 'Revisión app final incorrecta');
...
assert.strictEqual(manifest.start_url, './index.html?v=4.20.95&r=20', 'start_url PWA incorrecto');
...
assert(sw.includes("const MODULE_CACHE_REV = '3'"), 'Cache revision final ausente');
assert(sw.includes("'./style.css?v=4.20.95&r=16'"), 'CSS final no precacheado');
assert(sw.includes("'./app.js?v=4.20.95&r=20'"), 'App final no precacheada');
